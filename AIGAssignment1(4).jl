...
Xtrain_scaled, μ, σ = scale_features(Xtrain);

# Transform the testing features by using the learned artifacts
Xtesting_scaled = transform_features(Xtesting, μ, σ);
...
#θ, 𝐉 = logistic_regression_sgd(Xtrain _scaled, y_train, 0.0001, true, 0.3, 3000);
...
println(Accuracy(w, Xtrain, y_train))
println(Accuracy(w, Xtesting, y_test))
println("")
